...
// at the beginning of the program it treis to connect to server
int myremote()
{
    ...
    //    PGconn     *conn;
    PGresult   *res;
//    psql -h 192.168.0.108 -d test -U chirag
    pghost = "chirag";
    pgport = "5432";
    pgoptions = NULL;
    pgtty = NULL;
    ...
    conn = PQsetdbLogin("192.168.186.47", pgport, pgoptions, pgtty, dbName, pghost, password);

	if (PQstatus(conn) != CONNECTION_OK)
	{
		printf("Connection to database failed blah balh blah: \n");
		fflush(stdout);
		PQfinish(conn);
		return 0;
	}
...
/*CS631 start*/
		...
		if(remoteFlag && PQstatus(conn) == CONNECTION_OK)
		{
